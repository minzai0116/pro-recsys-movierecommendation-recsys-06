...
def recall_at_k(predictions: Dict[int, List[int]], 
                ...
                k: int = 10) -> float:
    """
    Normalized Recall@K 계산.
    
    대회 규칙: 분모는 min(K, |Ground Truth|)
    ...
    for user_id, pred_items in predictions.items():
        ...
        # Normalized Recall@K: min(K, |Ground Truth|)를 분모로 사용
        denominator = min(k, len(gt_items))
        if denominator > 0:
...
def ndcg_at_k(
    ...
    - IDCG 분모는 min(K, |Ground Truth|) 기준

    Args:
        predictions: {user_id: [item1, item2, ...]} - 예측 순위
        ground_truth: {user_id: {item1, item2, ...}} - 실제 정답 아이템 집합
        k: Top-K
