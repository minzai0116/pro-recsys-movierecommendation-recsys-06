...
class SelfAttention(nn.Module):
    def __init__(self, args):
        ...
        # 에러 처리 : hidden_size가 head 갯수로 안 나눠지면 오류 발생
        # 나중에 concat해서 그럼
    ...
    def transpose_for_scores(self, x):
        # [Batch, feature, embedding] >> [Batch, feature, head, head_size]
        # embedding을 head_size의 head로 나눔
        new_x_shape = x.size()[:-1] + (
            self.num_attention_heads,
            self.attention_head_size,
        )
        x = x.view(*new_x_shape)
        # [Batch, feature, head, head_size] >> [Batch, head, feature, head_size]
    ...
    def forward(self, input_tensor, attention_mask):
        ...
        # mask에 해당하는 자리는 -1e8값을 넣어준다. == 사실상 0
...
class Encoder(nn.Module):
    ...
    # hidden_state = [Batch, feature, Emb_dim]
    # attention_mask = [Batch, 1, 1, feature]
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True):
        all_encoder_layers = []
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, attention_mask) # [Batch, Seq, Emb_dim]
