...
@register_recbole_recipe("MultiVAE")
def build(cfg: Any) -> RecBoleRecipeBase:
    return MultiVAERecipe(cfg)


class MultiVAERecipe(RecBoleRecipeBase):
    """
    RecBole MultiVAE recipe.

    - 데이터: .inter만 필요(implicit top-n)
    - 학습 하이퍼: cfg.train.* (strict)
    - 모델 하이퍼: cfg.model_args.MultiVAE (있으면 overrides에 병합)
    """
